<template>
      ...
      <span >
        <div>
          {{ scope.row.fileName }}
              ...
              <el-table-column  prop="typename" label="文件类型" width="130px" ></el-table-column>
          </el-table>
  ...
  <script lang="ts" setup>
      ...
      import { $queryTypefile} from '../../api/file'
...
const background = ref(true)
  ...
  try {
  const supportedTypes = ['.txt', '.docx', '.doc', '.pdf', '.ppt', '.pptx', '.xls', '.xlsx', '.dll','.md','.exe',''];
  files.value = [];
  total.value = 0;
    // 构建多个查询请求
